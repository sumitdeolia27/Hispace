...
const hostUsersString = process.env.HOST_USERS || '[{"username": "hispace", "password": "P2P"}]';
...
// JWT config
const jwtCfg = {
    JWT_KEY: process.env.JWT_KEY || 'hispace_jwt_secret',
    JWT_EXP: process.env.JWT_EXP || '1h',
};

// Room presenters
const roomPresentersString = process.env.PRESENTERS || '["hispace P2P"]';
...
const api_key_secret = process.env.API_KEY_SECRET || 'hispacep2p_default_secret';
...
const statsData = {
    ...
    src: process.env.STATS_SCR || 'https://stats.hispace.com/script.js',
...
app.get('/join/', async (req, res) => {
    if (Object.keys(req.query).length > 0) {
        log.debug('Request Query', req.query);
        /* 
            http://localhost:3000/join?room=test&name=hispace&audio=1&video=1&screen=0&chat=1&notify=0&hide=0
            https://p2p.hispace.com/join?room=test&name=hispace&audio=1&video=1&screen=0&chat=1&notify=0&hide=0
            https://hispace.up.railway.app/join?room=test&name=hispace&audio=1&video=1&screen=0&chat=1&notify=0&hide=0
        */
...
app.get(`${apiBasePath}/stats`, (req, res) => {
    ...
    if (!api.isAuthorized()) {
        log.debug('hispace get stats - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
    ...
    log.debug('hispace get stats - Authorized', {
        header: req.headers,
        body: req.body,
        timestamp,
        totalRooms,
...
app.post(`${apiBasePath}/token`, (req, res) => {
    ...
    const api = new ServerApi(host, authorization, api_key_secret);
    if (!api.isAuthorized()) {
        log.debug('hispace get token - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
        return res.status(403).json({ error: 'Unauthorized!' });
    ...
    // log.debug the output if all done
    log.debug('hispace get token - Authorized', {
        header: req.headers,
        body: req.body,
        token: token,
    });
...
app.get(`${apiBasePath}/meetings`, (req, res) => {
    ...
    const api = new ServerApi(host, authorization, api_key_secret);
    if (!api.isAuthorized()) {
        log.debug('hispace get meetings - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
        return res.status(403).json({ error: 'Unauthorized!' });
    ...
    // log.debug the output if all done
    log.debug('hispace get meetings - Authorized', {
        header: req.headers,
        body: req.body,
        meetings: meetings,
    });
...
app.post(`${apiBasePath}/meeting`, (req, res) => {
    ...
    const api = new ServerApi(host, authorization, api_key_secret);
    if (!api.isAuthorized()) {
        log.debug('hispace get meeting - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
        return res.status(403).json({ error: 'Unauthorized!' });
    ...
    log.debug('hispace get meeting - Authorized', {
        header: req.headers,
        body: req.body,
        meeting: meetingURL,
    });
...
app.post(`${apiBasePath}/join`, (req, res) => {
    ...
    const api = new ServerApi(host, authorization, api_key_secret);
    if (!api.isAuthorized()) {
        log.debug('hispace get join - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
        return res.status(403).json({ error: 'Unauthorized!' });
    ...
    log.debug('hispace get join - Authorized', {
        header: req.headers,
        body: req.body,
        join: joinURL,
    });
...
app.get(`${apiBasePath}/activeRooms`, (req, res) => {
    ...
    // log.debug the output if all done
    log.debug('hispace get active rooms - Authorized', {
        header: req.headers,
        body: req.body,
