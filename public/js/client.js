...
/**
 * hispace P2P - Client component
 *
 * @link    GitHub: https://github.com/miroslavpejic85/hispace
 * @link    Official Live demo: https://p2p.hispace.com
 * @license For open source use: AGPLv3
 * @license For commercial use or closed source, contact us at license.hispace@gmail.com or purchase directly from CodeCanyon
 * @license CodeCanyon: https://codecanyon.net/item/hispace-p2p-webrtc-realtime-video-conferences/38376661
...
const images = {
    ...
    about: '../images/hispace-logo.gif',
    feedback: '../images/feedback.png',
    forbidden: '../images/forbidden.png',
    avatar: '../images/hispace-logo.png',
...
const themeSelect = getId('hispaceTheme');
...
async function whoAreYou() {
    ...
    Swal.fire({
        ...
        allowEscapeKey: false,
        background: swBg,
        title: brand.app?.name || 'hispace P2P',
        position: 'center',
        input: 'text',
            ...
            switch (event.channel.label) {
                case 'hispace_chat_channel':
                    try {
                        ...
                        console.error('hispace_chat_channel', err);
                    }
                    break;
                case 'hispace_file_sharing_channel':
                    try {
                        ...
                        if (dataFile instanceof ArrayBuffer && dataFile.byteLength != 0) {
                            ...
                            if (dataFile instanceof Blob && dataFile.size != 0) {
                                blobToArrayBuffer(dataFile)
                                    ...
                                    .catch((error) => {
                                        console.error('hispace_file_sharing_channel', error);
                                    });
                            }
                        }
                    } catch (err) {
                        console.error('hispace_file_sharing_channel', err);
                    }
                    break;
                default:
                    break;
                    ...
                    .then((local_description) => {
                        ...
                        pc.setLocalDescription(local_description)
                            .then(() => {
                                ...
                                // https://github.com/miroslavpejic85/hispace/issues/110
...
/**
 * Set hispace theme | dark | grey | ...
 */
function setTheme() {
    if (themeCustom.keep) return setCustomTheme();

    hispaceTheme.selectedIndex = lsSettings.theme;
    const theme = hispaceTheme.value;
    switch (theme) {
        case 'dark':
            // dark theme
...
function shareRoomByEmail() {
    Swal.fire({
        ...
        preConfirm: () => {
            ...
            const email = '';
            const emailSubject = `Please join our hispace P2P Video Chat Meeting`;
...
function createChatDataChannel(peer_id) {
    chatDataChannels[peer_id] = peerConnections[peer_id].createDataChannel('hispace_chat_channel');
...
function createFileSharingDataChannel(peer_id) {
    fileDataChannels[peer_id] = peerConnections[peer_id].createDataChannel('hispace_file_sharing_channel');
    ...
    if (isImageFile(incomingFileInfo.file.fileName)) {
        ...
        reader.onload = (e) => {
            Swal.fire({
                ...
                imageAlt: 'hispace-file-img-download',
                showDenyButton: true,
        ...
        Swal.fire({
            allowOutsideClick: false,
            background: swBg,
            imageAlt: 'hispace-file-download',
            imageUrl: images.share,
            position: 'center',
            title: 'Received file',
            text: incomingFileInfo.file.fileName + ' size ' + bytesToSize(incomingFileInfo.file.fileSize),
...
function showAbout() {
    ...
    Swal.fire({
        background: swBg,
        position: 'center',
        title: brand.about?.title && brand.about.title.trim() !== '' ? brand.about.title : 'WebRTC P2P v1.7.12',
...
function leaveFeedback() {
    Swal.fire({
        ...
        text: 'Do you want to rate your hispace experience?',
        confirmButtonText: `Yes`,
        denyButtonText: `No`,
        cancelButtonText: `Cancel`,
        showClass: { popup: 'animate__animated animate__fadeInDown' },
