...
/**
 * hispace Widget Factory - Main class for creating and managing widgets
 */
class hispaceWidget {
    static DEFAULT_OPTIONS = {
        ...
        supportWidget: {
            ...
            customMessages: {
                heading: 'Need a hand?',
                subheading:
                    'Hop on a <span style="font-weight: bold">Free 1:1 or Group Consultation</span> with a hispace Expert right now!',
                connectText: 'connect in < 10 seconds',
                onlineText: 'We are online',
                offlineText: 'We are offline',
                poweredBy: 'Powered by <span class="hispace-powered-by">hispace</span>',
            },
        },
    };
    ...
    constructor(domain, roomId, userName, options = {}) {
        ...
        this.options = this.mergeDeep(hispaceWidget.DEFAULT_OPTIONS, options);
    ...
    initWidgetRegistry() {
        if (!window.hispaceWidgets) {
            window.hispaceWidgets = new Map();
        }
    }
    ...
    createSupportWidgetHTML() {
        ...
        widget.className = `hispace-support-widget ${position} ${theme === 'light' ? 'light-theme' : ''}`;
        ...
        return `
            <div class="online-indicator">
                ...
                <div class="widget-controls">
                    <button class="minimize-btn" onclick="hispaceWidgetAction('minimize', this)" title="Minimize">
                        <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M19 13H5v-2h14v2z"/>
                        </svg>
                    </button>
                    <div class="close-btn" onclick="hispaceWidgetAction('close', this)" title="Close">&times;</div>
                </div>
            </div>
    ...
    createActionButtons() {
        ...
        return buttons
            .map(
                (btn) => `
            <button class="btn" onclick="hispaceWidgetAction('${btn.action}', this)">
    ...
    minimizeWidget() {
        const widget = document.querySelector('.hispace-support-widget');
        if (!widget) {
    ...
    restoreWidget() {
        const widget = document.querySelector('.hispace-support-widget');
        const minimizedBtn = document.querySelector('.hispace-minimized-btn');
    ...
    reopenWidget() {
        const reopenerBtn = document.querySelector('.hispace-reopener-btn');
    ...
    createMinimizedButton() {
        this.removeExistingElement('.hispace-minimized-btn');
    ...
    createReopenerButton() {
        this.removeExistingElement('.hispace-reopener-btn');
    ...
    updateOnlineStatus(online) {
        ...
        const elementsToUpdate = ['.hispace-support-widget', '.hispace-minimized-btn'];
    ...
    registerWidget(widgetId, element) {
        window.hispaceWidgets.set(widgetId, this);
    ...
    removeAllWidgetElements() {
        const selectors = ['.hispace-support-widget', '.hispace-minimized-btn', '.hispace-reopener-btn'];
    ...
    getParentNode() {
        return document.querySelector('#hispace-widget') || document.body;
    }
    ...
    getWidgetFromElement(element) {
        const widget = element.closest('.hispace-support-widget, .hispace-minimized-btn, .hispace-reopener-btn');
        if (widget) {
            const widgetId = widget.getAttribute('data-widget-id');
            return window.hispaceWidgets ? window.hispaceWidgets.get(widgetId) : this;
        }
        return this;
    }
    ...
    destroy() {
        ...
        if (window.hispaceWidgets) {
            for (const [key, widget] of window.hispaceWidgets.entries()) {
                if (widget === this) {
                    window.hispaceWidgets.delete(key);
                    break;
                }
            }
        }
...
window.hispaceWidgetAction = function (action, element) {
    try {
        const widgetElement = element.closest(
            '.hispace-support-widget, .hispace-minimized-btn, .hispace-reopener-btn'
        );
        ...
        const widget = window.hispaceWidgets?.get(widgetId);
...
document.addEventListener('DOMContentLoaded', function () {
    const autoInit = document.querySelector('[data-hispace-auto]');
    if (!autoInit) return;

    try {
        const buttonsAttr = autoInit.getAttribute('data-buttons');
        let buttonsConfig = { ...hispaceWidget.DEFAULT_OPTIONS.supportWidget.buttons };
        if (buttonsAttr) {
            // Normalize and map
        ...
        const config = {
            ...
            theme: autoInit.getAttribute('data-theme') || hispaceWidget.DEFAULT_OPTIONS.theme,
            widgetState: autoInit.getAttribute('data-widget-state') || hispaceWidget.DEFAULT_OPTIONS.widgetState,
            draggable:
                autoInit.getAttribute('data-draggable') === 'true' ||
                hispaceWidget.DEFAULT_OPTIONS.supportWidget.draggable,
            position: autoInit.getAttribute('data-position') || hispaceWidget.DEFAULT_OPTIONS.supportWidget.position,
            ...
            expertImages: autoInit.getAttribute('data-expert-images')
                ? autoInit
                      ...
                      .map((url) => url.trim())
                      .filter(Boolean)
                : hispaceWidget.DEFAULT_OPTIONS.supportWidget.expertImages,
            customMessages: {
                heading:
                    autoInit.getAttribute('data-heading') ||
                    hispaceWidget.DEFAULT_OPTIONS.supportWidget.customMessages.heading,
                subheading:
                    autoInit.getAttribute('data-subheading') ||
                    hispaceWidget.DEFAULT_OPTIONS.supportWidget.customMessages.subheading,
                connectText:
                    autoInit.getAttribute('data-connect-text') ||
                    hispaceWidget.DEFAULT_OPTIONS.supportWidget.customMessages.connectText,
                onlineText:
                    autoInit.getAttribute('data-online-text') ||
                    hispaceWidget.DEFAULT_OPTIONS.supportWidget.customMessages.onlineText,
                offlineText:
                    autoInit.getAttribute('data-offline-text') ||
                    hispaceWidget.DEFAULT_OPTIONS.supportWidget.customMessages.offlineText,
                poweredBy:
                    autoInit.getAttribute('data-powered-by') ||
                    hispaceWidget.DEFAULT_OPTIONS.supportWidget.customMessages.poweredBy,
            },
        };

        if (config.domain) {
            new hispaceWidget(config.domain, config.roomId, config.userName, {
                widgetState: config.widgetState,
                theme: config.theme,
                supportWidget: {
                    ...hispaceWidget.DEFAULT_OPTIONS.supportWidget,
        ...
        console.error('Failed to auto-initialize hispace Widget:', error);
    }
});
